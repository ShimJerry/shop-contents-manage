...
describe('ContentService', () => {
    ...
    it ("컨텐츠의 메타데이터를 수정할 수 있어야 한다.", () => {
        expect(contentService.updateContentMeta({title: "Updated Title"}));
        expect(contentService.getContent().title).toBe("Updated Title");
    })

    it ("컴포넌트 컬렉션을 가져올 수 있어야 한다.", () => {
        const componentCollection = contentService.getComponents();
        expect(componentCollection).toBeInstanceOf(ComponentCollection)
    })
})
